...
During **vvv{yy}** school, teachers will be giving frontal lessons and hands-on sessions regarding diverse courses, e.g. **kinematics**, **dynamics**, **vision** and so on. To this end, we make use of the [**GitHub Education**](https://education.github.com) platform.
...
- **GitHub Pages** are already enabled by default, given the name of the repository. The file **`_config.yml`** used by GitHub Pages to set up the style should be already contained inside the repository (thanks to duplication).
