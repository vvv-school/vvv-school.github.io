...
- Additionally, you have to be a **OAuth user** with `read:org` and `repo:status` scopes to retrieve from [GitHub API](https://developer.github.com/v3) reserved information regarding teams, create commit statuses as well as increase the _rate limit_ of our requests. To do so, after you created a [personal access token on GitHub](https://help.github.com/articles/creating-an-access-token-for-command-line-use), save the token in the environment variable **GITHUB_TOKEN_ORG_READ**:
